classdef rsigly
    ...
    methods 
        function obj = rsigly(datafilter,period) %period is RSI variable
            if ~isa(datafilter,'datafilter')
                error('datafilter must be a datafilter class object')
            end
            ret=datafilter.ret;
            high=datafilter.high_price;
            low=datafilter.low_price;
            close=datafilter.close_price;
            hlc3=(high+low+close)/3;
            [row,col]=size(close);
            %RSI
            rsi=rsindex(close,period);
            ...
            vol=nan(length(close),1);
            Kstochastic=nan(length(close),1);
            Dstochastic=nan(length(close),1);
            SlowDstochastic=nan(length(close),1);
