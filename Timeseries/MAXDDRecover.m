function [MDD, MDDs, MDDe, MDDr, RecoverPeriod] = MAXDDRecover(r)
...
%          aReturnVector
% RecoverPeriod: number of days to recover the drawdown
...
MDD = max(dd);
MDDe = find(dd==MDD);
MDDs = find(abs(cr(MDDe)+ MDD - cr) < 0.000001); MDDs=MDDs(1);
MDDr = MDDe+min(find(cr(MDDe:end) >= cr(MDDs)))-1;
RecoverPeriod=MDDr-MDDe;